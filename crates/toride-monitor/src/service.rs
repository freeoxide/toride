...
mod tests {
    ...
    fn is_active_issues_systemctl_is_active_for_monitor_unit() {
        ...
        // systemctl is-active probe routed through the runner. `run_systemctl`
        // injects a literal `--` before the unit name (so a unit starting with
        // `-` can't masquerade as a flag), so the registered spec must mirror
