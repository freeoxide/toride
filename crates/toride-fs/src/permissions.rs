...
//!
//! The helpers here express permissions as raw Unix mode bits (`u32`) and
//! ownership as a uid, which have no direct Windows equivalent. The POSIX-mode
//! API ([`set_permissions`], [`check_not_world_writable`],
//! [`check_owner_is_root`]) is therefore **Unix-only** and is compiled only on
//! Unix targets. Windows ACL equivalents are deliberately out of scope.
//!
...
mod posix {
    ...
    ///
    /// This function is Unix-only: it takes raw POSIX mode bits and maps to
    /// `chmod(2)`.
    ///
    ...
    ///
    /// This function is Unix-only: it inspects the POSIX "other" write bit.
    ///
    ...
    ///
    /// This function is Unix-only: ownership is expressed as a POSIX uid.
    ///
...
#[cfg(unix)]
pub use posix::{check_not_world_writable, check_owner_is_root, set_permissions};
