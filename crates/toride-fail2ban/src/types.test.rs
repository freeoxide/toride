...
fn sample_log_entry() -> LogEntry {
    LogEntry {
        ...
        ip: Some(sample_ipv4()),
        line_number: 42,
        raw_line: "Failed password for root from 192.168.1.100".to_string(),
        jail_name: "sshd".to_string(),
    }
}
...
#[test]
fn log_entry_create_with_all_fields() {
    let entry = sample_log_entry();
    assert_eq!(entry.timestamp, Utc.with_ymd_and_hms(2025, 6, 15, 10, 0, 0).unwrap());
    assert_eq!(entry.ip, Some(sample_ipv4()));
    assert_eq!(entry.line_number, 42);
    assert_eq!(entry.raw_line, "Failed password for root from 192.168.1.100");
    assert_eq!(entry.jail_name, "sshd");
}

#[test]
fn log_entry_equality() {
    let a = sample_log_entry();
    let b = sample_log_entry();
    assert_eq!(a, b);
}

#[test]
fn log_entry_clone() {
    let original = sample_log_entry();
    let cloned = original.clone();
    assert_eq!(original, cloned);
}

#[test]
fn log_entry_with_no_ip() {
    let entry = LogEntry {
        ip: None,
        ..sample_log_entry()
    };
...
#[test]
fn log_entry_with_ipv6() {
    let entry = LogEntry {
        ip: Some(sample_ipv6()),
        ..sample_log_entry()
    };
    assert_eq!(entry.ip.unwrap(), sample_ipv6());
}

// --- Edge-case tests ------------------------------------------------------

#[test]
fn log_entry_line_number_zero() {
    let entry = LogEntry {
        line_number: 0,
        ..sample_log_entry()
    };
...
#[test]
fn log_entry_line_number_max() {
    let entry = LogEntry {
        line_number: u64::MAX,
        ..sample_log_entry()
    };
    assert_eq!(entry.line_number, u64::MAX);
}

#[test]
fn log_entry_empty_raw_line() {
    let entry = LogEntry {
        raw_line: String::new(),
        ..sample_log_entry()
    };
    assert_eq!(entry.raw_line, "");
}

#[test]
fn log_entry_empty_jail_name() {
    let entry = LogEntry {
        jail_name: String::new(),
        ..sample_log_entry()
    };
...
#[test]
fn log_entry_inequality_on_line_number() {
    let a = sample_log_entry();
    let b = LogEntry {
        line_number: 99,
        ..sample_log_entry()
...
fn log_entry_serialization_roundtrip() {
    let entry = sample_log_entry();
    let json = serde_json::to_string(&entry).expect("serialization failed");
    let deserialized: LogEntry = serde_json::from_str(&json).expect("deserialization failed");
    assert_eq!(entry, deserialized);
}

#[test]
fn log_entry_serialization_with_none_ip() {
    let entry = LogEntry {
        ip: None,
        ..sample_log_entry()
    };
...
#[test]
fn log_entry_deserialize_from_json_string() {
    let json = r#"{
    ...
    let entry: LogEntry = serde_json::from_str(json).expect("deserialization failed");
    assert!(entry.ip.is_none());
    assert_eq!(entry.line_number, 1);
    assert_eq!(entry.raw_line, "server started");
}
